...
//				g_err_A = 0x01;	
   	...
   	{     								  			// GPS数据包的格式是： GPZDA+时间数据； 所以先判断出GPZADA，然后将后边的时间数据提取出来；		      		
		...
		else if(g_couter==30)								// 校验码	  
		{
  	  	...
  	  	{ 
			//接收到最后一个字节，则认为本次连接上了，但是并不保证数据的正确性
			g_clr_count = 0;								
			...
			{												// dongweijie 2008-10-20
				if (g_gps_True==0x02&&g_errFlag==0x1A)
				{
					g_errFlag = 0x00;			   			// 说明本次传输无故障,或者是故障恢复
				}
...
void  key_up() using 2 
{
	if (g_adjust_flag==1)
	{
		...
		if(g_couter_adj==0) 							//year 0~99 
		{
			...
			if((leap_year_using2_i())&&(g_timecom_buffer[1]==2))  //如果年份为闰年，则如果月份是2的话，需要对日期进行处理，不然就会出错
			{
			 	if(g_timecom_buffer[2]<=29)//日期如果小于29则，日期不需要变化；否则修改为月初1号
				{
					;
				}
				else
				{
				 	g_timecom_buffer[2]=1;//闰年则设置2月份的日期号是1号
				}
			}                 
		} 
		else if (g_couter_adj==1)  						//month 1~12 
		{
			...
			if(g_timecom_buffer[1]==2)//如果是2月
			{
				if(g_timecom_buffer[2]<=28)
				{
				 	;//日期保持不变
				}
				else
				{
					if((g_timecom_buffer[2]==29)&&(leap_year_using2_i()))//日期为29天，且当年是闰年，则保持日期不变，否则日期修改为1
					{
					 	;//日期保持不变
					}
					else
					{
						g_timecom_buffer[2] = 1;//修改日期为1号
					}
				}
...
void  key_down() using 2 
{
	...
	{          
		if(g_couter_adj==0)  							//year 0~99  //
		{
			...
			if((leap_year_using2_i())&&(g_timecom_buffer[1]==2))  //如果年份为闰年，则如果月份是2的话，需要对日期进行处理，不然就会出错
			{
			 	if(g_timecom_buffer[2]<=29)//日期如果小于29则，日期不需要变化；否则修改为月末29号
				{
					;
				}
				else
				{
				 	g_timecom_buffer[2]=29;//闰年则设置2月份的日期号是29号
				}
			}             
		} 
		else if (g_couter_adj==1)  						//month 1~12//
		{
			...
			if(g_timecom_buffer[1]==2)//如果是2月
			{
				...
				{
					if((g_timecom_buffer[2]==29)&&(leap_year_using2_i()))//日期为29天，且当年是闰年，则保持日期不变，否则日期修改为29
					{
					 	;//日期保持不变
					}
					else
					{
						g_timecom_buffer[2] = 28;//修改日期为28号
					}
				}
